init()
{
	...
	precacheString( "gfx/hud/hud@health_cross.tga" );
	precacheString( &"|" );
	precacheString( &"^3+" );
	...
	while ( isAlive( self ) )
	{
		...
		if ( self.pers[ "team" ] == "axis" )
		{
			...
			if ( self.bodyarmor > 0 )
			{
				...
				self.bodyarmor_hud_front scaleOverTime( 0.5 );
			...
			{
				if ( self.explo_armor_hud_back.alpha == 0 )
				{
				...
				self.exploarmor_hud_front scaleOverTime( 0.5 );
...
darkness()
{
	...
	i = 0.00;
	stop = false;
	while ( isAlive( self ) && !self.nightvision )
	{
		if ( i > 0.60 )
			break;
